        ...
        <div class="row w-100">
            <div class="col-12 w-100">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 text-md-end text-center order-1 order-md-2 my-1 my-md-2">
                    ...
                    <div
                        class="col-12 d-flex flex-md-row flex-column justify-content-between text-md-start text-center order-3 my-2">
                    ...
                    <div class="col-12 d-flex justify-content-md-end justify-content-center align-items-center order-5">
                        <div class="text-end my-2 " v-if="!account.id">
                            ...
                            <button class="btn btn-valor button w-100" @click="login()">Register</button>
                        </div>
                        <div v-else class="my-2 text-end">
                            <RouterLink v-if="activeTournament.creatorId == account.id"
                                ...
                                <button class="btn btn-valor button w-100">Edit</button>
                            </RouterLink>
                            <div v-else class="text-end">
                                <button @click="registerForTournament(players, account)"
                                    ...
                                    class="btn btn-valor button w-100">Register
                                </button>
                                <button @click="unregisterForTournament(players.find(p => p.accountId == account.id))"
                                    v-else
                                    :disabled="activeTournament.startDate?.toLocaleDateString() < new Date().toLocaleDateString()"
                                    class="btn btn-danger button w-100">Unregister
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    setup() {
        ...
        return {
            ...
            async unregisterForTournament(account) {
                try {
                    const yes = await Pop.confirm('Are you sure you would like to cancel your registration for this event?', 'If an entry fee was assessed during registration, you will receive a refund');
                    if (!yes) {
                        return;
                    }
